...
public class UnindexedMatcher {
    ...
    if (node instanceof AndQueryNode) {
        return executeAndQuery((AndQueryNode) node, rev);
    } else if (node instanceof OrQueryNode) {
        return executeOrQuery((OrQueryNode) node, rev);
    } else if (node instanceof OperatorExpressionNode) {
        return executeOperatorExpression((OperatorExpressionNode) node, rev);
    } else {
...
private boolean executeAndQuery(AndQueryNode andNode, DocumentRevision rev) {
...
private boolean executeOrQuery(OrQueryNode orNode, DocumentRevision rev) {
...
@SuppressWarnings("unchecked")
private boolean executeOperatorExpression(OperatorExpressionNode node, DocumentRevision rev) {
    ...
    return invertResult != performValueComparison(actual, operator, expected);
}


private boolean performValueComparison(Object actual, String operator, Object expected) {
   List<String> specialCaseOperators = Arrays.asList(QueryConstants.MOD, QueryConstants.SIZE);
    if (specialCaseOperators.contains(operator)) {
        ...
        return handleRegularComparison(actual, operator, expected);
...
@SuppressWarnings("unchecked")
private boolean handleRegularComparison(Object actual, String operator, Object expected) {
...
// Existing valueCompare, compareEq, compareLT, compareLTE, compareGT, compareGTE, compareMOD, and compareSIZE methods remain unchanged.
