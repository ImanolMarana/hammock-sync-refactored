...
public class CouchClient {
    ...
    try {
        ...
        if (!Arrays.asList(defSslCtx.getDefaultSSLParameters().getProtocols()).contains(TlsOnlySslSocketFactory.TLSv12) &&
            Arrays.asList(defSslCtx.getSupportedSSLParameters().getProtocols()).contains(TlsOnlySslSocketFactory.TLSv12)) {
    ...
    } catch (NoSuchAlgorithmException | KeyManagementException | ClassNotFoundException | 
...
private static int getAndroidApiLevel() throws ClassNotFoundException, NoSuchFieldException, IllegalAccessException {
