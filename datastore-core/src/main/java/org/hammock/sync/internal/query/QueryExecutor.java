...
class QueryExecutor {
    ...
    protected Set<String> executeQueryTree(QueryNode node, SQLDatabase db) throws QueryException {
        if (node instanceof AndQueryNode) {
            return executeAndQuery((AndQueryNode) node, db);
        } else if (node instanceof OrQueryNode) {
            return executeOrQuery((OrQueryNode) node, db);
        } else if (node instanceof SqlQueryNode) {
            return executeSqlQuery((SqlQueryNode) node, db);
        } else {
            throw new QueryException("Unexpected node type " + node);
        }
    }

    private Set<String> executeAndQuery(AndQueryNode node, SQLDatabase db) throws QueryException {
        Set<String> accumulator = null;

        for (QueryNode qNode : node.children) {
    ...
    private Set<String> executeOrQuery(OrQueryNode node, SQLDatabase db) throws QueryException {
        Set<String> accumulator = null;
    ...
    private Set<String> executeSqlQuery(SqlQueryNode node, SQLDatabase db) throws QueryException {
        try {
            List<String> docIds = null;

            if (node.sql != null) {
                docIds = executeSqlWithPlaceholders(node.sql, db);
            } else {
    ...
    private List<String> executeSqlWithPlaceholders(SqlParts sqlParts, SQLDatabase db) throws QueryException {
