...
public class DatabaseImpl implements Database, org.hammock.sync.documentstore.advanced.Database {
    ...
    try {
        DocumentRevisionTree docTree = fetchDocumentRevisionTree(docId);
        if (!docTree.hasConflicts()) {
            return;
        }

        DocumentRevision newWinner = resolveConflictsWithResolver(resolver, docId, docTree);
        if (newWinner == null) {
            return;
        }
        ...
        get(queue.submitTransaction(new ResolveConflictTransaction(docTree, revIdKeep, newWinnerTx, attachmentsDir, attachmentStreamFactory)));
...
private DocumentRevisionTree fetchDocumentRevisionTree(String docId) throws ExecutionException {
    return get(queue.submit(new GetAllRevisionsOfDocumentCallable(docId, attachmentsDir, attachmentStreamFactory)));
}


private DocumentRevision resolveConflictsWithResolver(ConflictResolver resolver, String docId, DocumentRevisionTree docTree) {
...
private class ResolveConflictTransaction implements SQLCallable<Void> {
    private final DocumentRevisionTree docTree;
    private final String revIdKeep;
    private final DocumentRevision newWinnerTx;
    private final String attachmentsDir;
    private final AttachmentStreamFactory attachmentStreamFactory;


    public ResolveConflictTransaction(DocumentRevisionTree docTree, String revIdKeep, DocumentRevision newWinnerTx, String attachmentsDir, AttachmentStreamFactory attachmentStreamFactory) {
        this.docTree = docTree;
        this.revIdKeep = revIdKeep;
        this.newWinnerTx = newWinnerTx;
    ...
    public Void call(SQLDatabase db) throws Exception {
        ...
        boolean isNewOrUpdatedRevision = isNewOrUpdatedRevision(newWinnerTx);
        if (isNewOrUpdatedRevision) {
            updateDocumentWithAttachments(db, newWinnerTx);
        }
        return null;
    }

    private boolean isNewOrUpdatedRevision(DocumentRevision newWinnerTx) {
        ...
        if (newWinnerTx.getClass().equals(DocumentRevision.class)) {
            ...
            return newWinnerTxInternal.isBodyModified() ||
                    (newWinnerTxInternal.getAttachments() != null && newWinnerTxInternal.getAttachments().hasChanged());

        }
        return false;
    }

    private void updateDocumentWithAttachments(SQLDatabase db, DocumentRevision newWinnerTx) throws Exception {
