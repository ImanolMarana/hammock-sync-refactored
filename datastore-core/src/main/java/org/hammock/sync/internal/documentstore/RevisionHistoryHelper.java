...
public class RevisionHistoryHelper {
    ...
    private static void addAttachments(Map<String, ? extends Attachment> attachments,
                                     ...
                                     boolean shouldInline,
                                     int minRevPos) {
    LinkedHashMap<String, Object> attsMap = new LinkedHashMap<>();
    outMap.put("_attachments", attsMap);

    attachments = new TreeMap<>(attachments);
    for (Map.Entry<String, ? extends Attachment> att : attachments.entrySet()) {
        addAttachment(attsMap, att, shouldInline, minRevPos);
    }
}

private static void addAttachment(Map<String, Object> attsMap,
                                  Map.Entry<String, ? extends Attachment> att,
                                  boolean shouldInline, int minRevPos) {
    SavedAttachment savedAtt = (SavedAttachment) att.getValue();
    HashMap<String, Object> theAtt = new HashMap<>();
    try {
        if (savedAtt.revpos > minRevPos) {
            if (!shouldInline) {
                theAtt.put("follows", true);
            } else {
                inlineAttachment(savedAtt, theAtt);
            }
...
private static void inlineAttachment(SavedAttachment savedAtt, HashMap<String, Object> theAtt) throws IOException {
    ...
    theAtt.put("data", new String(baos.toByteArray(), "UTF-8"));
}
