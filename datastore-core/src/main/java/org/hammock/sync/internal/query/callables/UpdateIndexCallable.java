...
public class UpdateIndexCallable implements SQLCallable<Void> {
    ...
    List<FieldSort> includeFieldNames = new ArrayList<>(initialIncludedFields);
    ...
    for (FieldSort fieldName : fieldNames) {
        ...
        Object value = ValueExtractor.extractValueForFieldName(fieldName.field, rev.getBody());
        if (value != null && !isEmptyList(value)) {
    ...
    for (int argIndex = 0; argIndex < includeFieldNames.size(); argIndex++) {
        String fieldName = String.format("\"%s\"", includeFieldNames.get(argIndex).field);
        putValueInContentValues(contentValues, fieldName, args.get(argIndex));
    }
...
private void putValueInContentValues(ContentValues contentValues, String fieldName, Object argument) {
