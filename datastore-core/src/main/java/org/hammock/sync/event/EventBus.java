...
public class EventBus {
    ...
    validateListenerClass(listenerClass);

    List<SubscriberMethod> methods = findSubscriberMethods(listenerClass);

    listeners.put(object, methods);
}

private List<SubscriberMethod> findSubscriberMethods(Class<?> listenerClass) {
    List<SubscriberMethod> methods = new ArrayList<SubscriberMethod>();
    do {
        methods.addAll(extractSubscriberMethods(listenerClass));
    } while ((listenerClass = listenerClass.getSuperclass()) != null);
    return methods;
}

private List<SubscriberMethod> extractSubscriberMethods(Class<?> listenerClass) {
    List<SubscriberMethod> methods = new ArrayList<>();
    for (Method m : listenerClass.getDeclaredMethods()) {
        if (m.isAnnotationPresent(Subscribe.class)) {
            validateSubscriberMethod(listenerClass, m);
            Class<?>[] params = m.getParameterTypes();
            if (params.length == 1) {
                validateParameterType(listenerClass, m, params[0]);
...
private void validateListenerClass(Class<?> listenerClass) {
...
private void validateSubscriberMethod(Class<?> listenerClass, Method m) {
...
private void validateParameterType(Class<?> listenerClass, Method m, Class<?> paramType) {
