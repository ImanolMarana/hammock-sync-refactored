...
public class QueryResult implements Iterable<DocumentRevision> {
    ...
    try {
        ...
        while (range.location < originalDocIds.size() && !limitReached) {
            loadDocumentBatch(docList);
        }
...
private void loadDocumentBatch(List<DocumentRevision> docList) throws DocumentStoreException {
    ...
    for (DocumentRevision rev : docs) {
        if (shouldSkipDocument(rev)) {
            continue;
        }

        DocumentRevision innerRev = (fields != null && !fields.isEmpty()) ? projectFields(fields, rev, database) : rev;
...
private boolean shouldSkipDocument(DocumentRevision rev) {
    if (matcher != null && !matcher.matches(rev)) {
        return true;
    }
...
private boolean checkAndIncrementLimit() {
    nReturned++;
...
private DocumentRevision projectFields(List<String> fields, DocumentRevision rev, Database database) {
    ...
    return new DocumentRevisionBuilder()
            .setDocId(rev.getId())
            .setRevId(rev.getRevision())
            .setBody(DocumentBodyFactory.create(body))
            .setDeleted(rev.isDeleted())
            .setAttachments(rev.getAttachments())
